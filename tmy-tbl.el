;;; tmy-tbl.el --- Tmy-Code dependent data

;; Copyright (c) 2016 satocis <satocis@users.noreply.github.com>
...
;; This code is modified verion of tc-tbl.el v1.10 2002/12/18 02:03:31 which is
;; a part of TC2.
;; Modifed date is not sure. But publicly releaced at November 31, 2016.
;; TC2 is licenced under the terms of the GNU General Public License version 2
;; or any later version.

;;; Commentary:
;; also needs tmy-etbl.el
;;
...
(setq tcode-input-method 'tmycode)

(setq tcode-transparent-mode-indicator "--"
      tcode-tcode-mode-indicator "TM"
      tcode-alnum-2byte-tcode-mode-indicator "苫"
      tcode-hiragana-mode-indicator ""
      tcode-katakana-mode-indicator "")
...
(setq tcode-stroke-file-name (concat tcode-data-directory "tmycode.st"))
...
(let ((i 0))
  (while (< i 40)
    (setq tcode-special-commands-alist
		  (cons (cons (list i 40)
					  'tmy-code-verbose-stroke)
				tcode-special-commands-alist))
    (setq i (1+ i))))

(defun tmy-code-verbose-stroke ()
  (interactive)
...
(setq tcode-stroke-to-string-option 'tmy-code-stroke-to-string
      eelll-stroke-to-string-function 'tmy-code-stroke-to-string
      eelll-stroke-to-string-column 'tmy-code-stroke-to-string-column)

(defvar tmy-code-stroke-to-string-use-image nil)

(defvar tmy-code-stroke-to-string-option
...
(defun tmy-code-stroke-to-string (stroke)
  (let ((tcode-stroke-to-string-option tmy-code-stroke-to-string-option)
		(dat (tcode-stroke-prefix-match stroke)))
    (if (null dat)
		(tcode-stroke-to-string stroke)
      (let ((x (and (not tmy-code-stroke-to-string-use-image)
...
(defun tmy-code-stroke-to-string-column (stroke)
